...
test_path = "../data/storage"
image_paths = sorted(list(paths.list_images(test_path)))
item_model = keras.models.load_model("item_model.h5")
ocr_model = keras.models.load_model("ocr_model.h5")
for each in image_paths:
    print(each)
    # scene = cv2.imread(each)
    base64test = ""
    with open("base64test.txt", "r") as f:
        base64test = f.read()
    missing_padding = 4 - len(base64test) % 4
    if missing_padding:
        base64test += '=' * missing_padding
    imgData = base64.b64decode(base64test)
